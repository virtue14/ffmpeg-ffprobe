...
public record MediaMetadataResponse(
    ...
    /**
     * FFmpegProbeResult로부터 MediaMetadataResponse를 생성합니다.
     *
     * 포맷 필드와 스트림 정보를 DTO로 매핑하며, 스트림의 평균 프레임 속도(avg_frame_rate)가 없을 경우 "N/A"로 설정합니다.
     *
     * @param result 포맷 및 스트림 정보를 포함하는 FFprobe 결과 객체
     * @return 제공된 프로브 결과로부터 생성된 MediaMetadataResponse
     */
                ...
                result.getStreams().stream()
                        .map(
                                s -> new Stream(
                                        ...
                                        s.width,
                                        s.height,
                                        s.avg_frame_rate != null ? s.avg_frame_rate.toString() : "N/A"))
